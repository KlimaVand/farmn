...
//            globalTime.Instance.start(1,"Nless");
            ...
            if (N_Spring < 0)
            {
                message.Instance.addWarnings("N foraar er forkert. Den er " + N_Spring.ToString() + " men burde være over 0", "N_LES: N_Spring is not valid", 2);
                return -2;
            }
            if (N_Fall < 0)
            {
                message.Instance.addWarnings("N efteraar er forkert. Den er " + N_Fall.ToString() + " men burde være over 0", "N_LES: N_Fall is not valid", 2);
                return -3;
            }
            if (N_Fix < 0)
            {
                message.Instance.addWarnings("N fix er forkert. Den er " + N_Fix.ToString() + " men burde være over 0", "N_LES: N_Fix is not valid", 2);
                return -4;
            }
            if (N_GrazingManure < 0)
            {
                message.Instance.addWarnings("N afgraesning er forkert. Den er " + N_GrazingManure.ToString() + " men burde være over 0", "N_LES: N_GrazingManure is not valid", 2);
                return -5;
            }
            if (N_Removed < 0)
            {
                message.Instance.addWarnings("Fjernelse af N er forkert. Den er " + N_Removed.ToString() + " men burde være over 0", "N_LES: N_Removed is not valid", 2);
                return -6;
            }
            if (Year < 1963)
            {
                message.Instance.addWarnings("Aaret er forkert. Den er " + Year.ToString() + " men burde være over 1963", "N_LES: Year is not valid", 2);
                return -7;
            }
